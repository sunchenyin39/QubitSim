...
def main():
    ...
    individual_fedelity = SFQControl.quantum.Fedelity(SFQControl.quantum.UGgenerator(
        USFQ, UFR, individual, times, operator_order_num, dressed_eigenvalue, dressed_featurevector, T_SFQclock), matrix)
    print(individual_fedelity)
    it_num = 10000
    mut_size = 20
    for i in range(it_num):
        index_list = np.random.randint(len(individual), size=mut_size)
        individual_new = copy.deepcopy(individual)
        for j in range(len(index_list)):
            individual_new[index_list[j]] = (individual_new[j]+1) % 2
        individual_new_fedelity = SFQControl.quantum.Fedelity(SFQControl.quantum.UGgenerator(
            USFQ, UFR, individual_new, times, operator_order_num, dressed_eigenvalue, dressed_featurevector, T_SFQclock), matrix)
        if individual_new_fedelity > individual_fedelity:
            np.save("individual.npy", individual_new)
            print(individual_new_fedelity)
            break
