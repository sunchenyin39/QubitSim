...
circuit.signal_2 = Amplitude*Envolope*np.cos(2*np.pi*f01_Q2*circuit.t_list+phi)
plt.figure()
plt.plot(circuit.t_list*1E9, circuit.signal_2)
plt.title("signal_2")
plt.xlabel("t/ns")
plt.tight_layout()
plt.savefig(fname="idleQ1_X2PQ2_signal.png")
# 3.Run.
circuit.run()
# 4.Matrix display
idleQ1_X2PQ2_matrix = np.load(circuit.npy_filename)
print("idleQ1_X2PQ2_matrix:")
for i in range(4):
    for j in range(4):
        print("%.4f" % np.abs(idleQ1_X2PQ2_matrix[i][j]), end='_')
        print("%.4f" % np.angle(idleQ1_X2PQ2_matrix[i][j]), end=',')
    print()

X2PQ2_matrix = np.zeros([2, 2], dtype=complex)
X2PQ2_matrix[0][0] = idleQ1_X2PQ2_matrix[0][0]
X2PQ2_matrix[0][1] = idleQ1_X2PQ2_matrix[0][1]
X2PQ2_matrix[1][0] = idleQ1_X2PQ2_matrix[1][0]
X2PQ2_matrix[1][1] = idleQ1_X2PQ2_matrix[1][1]
print("\nX2PQ2_matrix:")
print(X2PQ2_matrix)

theta_g = (np.angle(X2PQ2_matrix[0][0])+np.angle(X2PQ2_matrix[1][1]))/2.0
phi = 2*np.arccos(np.real(X2PQ2_matrix[0][0]/np.exp(complex(0, 1)*theta_g)))
nx = np.imag(X2PQ2_matrix[0][1] /
             np.exp(complex(0, 1)*theta_g))/(-1)/np.sin(phi/2)
ny = np.real(X2PQ2_matrix[0][1] /
             np.exp(complex(0, 1)*theta_g))/(-1)/np.sin(phi/2)
nz = np.imag(X2PQ2_matrix[0][0] /
             np.exp(complex(0, 1)*theta_g))/(-1)/np.sin(phi/2)
